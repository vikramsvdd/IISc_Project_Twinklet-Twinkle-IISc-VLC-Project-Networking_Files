...
width_list=[10, 17, 17, 25, 25, 17, 15, 20, 20]

#bg_list=[ 'light sky blue', 'light gray', 'light sky blue', 'light gray', 'light sky blue']
bg_list=[ 'light sky blue', 'light gray', 'light sky blue', 'light gray', 'light sky blue', 'light gray', 'light sky blue','light gray','light sky blue' ]
...
def Refreshingfun():
    ...
    print(iter_count)
    if iter_count<yc:
            cursor = conn.execute("SELECT SRNO, TWINKLERID, FLASHPOINTID, FLASHPOS, DATETIME, DATA_PACK, REMARKS, DAILY_DELAY_STATISTICS, CUMULATIVE_DELAY_STATISTICS from TWINKLE_DATA WHERE SRNO<:SRNO", {"SRNO": (iter_count+1)})
    else:
            cursor = conn.execute("SELECT SRNO, TWINKLERID, FLASHPOINTID, FLASHPOS, DATETIME, DATA_PACK, REMARKS, DAILY_DELAY_STATISTICS, CUMULATIVE_DELAY_STATISTICS from TWINKLE_DATA WHERE SRNO BETWEEN ? AND ?", ((iter_count-(yc-2)), iter_count))
    rowx = 0
    for row in cursor:
...
class Window(Tk):
    ...
    def initialize(self):
        ...
        self.button2.place(x=500, y=125)
    ...
    def OnButtonClick2(self):
        ...
        t.setfir(0,7,"Daily Delay Stats")
        t.setfir(0,8,"Cumulative Delay Stats")
